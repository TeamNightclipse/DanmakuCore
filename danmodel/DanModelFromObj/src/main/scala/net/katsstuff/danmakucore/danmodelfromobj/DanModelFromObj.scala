...
object DanModelFromObj {
    ...
    Config.parser.parse(args, Config()) match {
      case Some(cfg) =>
        ...
        val tried = Try {
          ...
          val materials = model.getMaterialLibraries.asScala.map { str =>
            val mtlFile = directory.resolve(str)
            mtlParser.parse(Files.newInputStream(mtlFile))
          }.flatMap(_.getMaterials.asScala).groupBy(_.getName).map { case (name, innerMaterials) =>
  ...
  def createDanModel(cfg: Config, model: OBJModel, materials: Map[String, MTLMaterial]): Array[Byte] = {
    ...
    objects.foreach { obj =>
      ...
      obj.getMeshes.asScala.foreach { mesh =>
        ...
        val material = materials.getOrElse(mesh.getMaterialName, error(s"Found object ${obj.getName} where parts had no material. Tried to find ${mesh.getMaterialName}"))
  ...
  def writeMesh(model: OBJModel, mesh: OBJMesh, material: MTLMaterial, isDanmakuColor: Boolean): (Array[Byte], GLMode) = {
    ...
    if(!faceReferences.forall(_.size == faceReferences.head.size)) error("Found model with varying amount of vertices for faces. Stick to one amount")

    val maxSides = faceReferences.head.size
    val glMode = if(maxSides == 3) GLMode.GLTriangles else if(maxSides == 4) GLMode.GLQuads else error("Found model with faces with more than 4 vertices. It's ineffcient. Fix it")
