...
object DanOBJParser extends JavaTokenParsers {
  ...
  def read(string: String): Either[String, (Seq[OptimizedTriangleData], ResourceLocation)] = {
    ...
    val triangles = parse(version, string).flatMapWithNext {
      case 1 =>
        parse(newLine ~> danmakuColor, _)
          .flatMapWithNext(
            danmakuMarkerColor =>
              parse(newLine ~> glowColor, _)
                .flatMapWithNext(
                  glowMarkerColor =>
                    parse(newLine ~> texture, _)
                      .flatMapWithNext(
                        texture =>
                          parse(blankLine ~> allVertices, _)
                            .flatMapWithNext(
                              pos =>
                                parse(blankLine ~> allTextures, _)
                                  .flatMapWithNext(
                                    uv =>
                                      parse(blankLine ~> allColors, _)
                                        .flatMapWithNext(
                                          color =>
                                            parse(blankLine ~> allNormals, _)
                                              .flatMapWithNext(
                                                norm =>
                                                  parse(blankLine ~> allFaces, _)
                                                    .map(faces => {
                                                      ...
                                                      @tailrec
                                                      def inner(
                                                          rest: Seq[(FaceInfo, FaceInfo, FaceInfo)],
                                                          acc: Seq[OptimizedTriangleData]
                                                      ): Seq[OptimizedTriangleData] = {
                                                        if (rest == Nil) acc
                                                        else {
                                                          ...
                                                          val triangle =
                                                            TriangleData(vert1, vert2, vert3).optimize(mappedGlowColor, mappedDanmakuColor)
