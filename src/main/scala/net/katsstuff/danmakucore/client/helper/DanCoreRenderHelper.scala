...
import net.katsstuff.mirror.client.shaders.{MirrorShaderProgram, MirrorUniform, ShaderManager, ShaderType, UniformBase, UniformType}
...
object DanCoreRenderHelper {
  ...
  def danmakuShaderProgram: Option[MirrorShaderProgram] = ShaderManager.getShaderProgram(baseDanmakuShaderLoc)
  ...
  def updateDanmakuShaderAttributes(shaderProgram: MirrorShaderProgram, form: IRenderForm, shot: ShotData): Unit = {
    ...
    attributeMap.flatMap { t =>
      val res: Option[((Option[Char], Float), MirrorUniform)] = shaderProgram.getUniform(t._1).map(t._2 -> _)
      res
    }.foreach {
